...
- Complete the `cast_fireball` method:
    1. [ ] If there isn't enough mana to cast a fireball (see `fireball_cost` at the top of the file), `raise` an `Exception` with the message `____ cannot cast fireball`, where `____` is the wizard's name.
